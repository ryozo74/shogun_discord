...
discord_listener.sh runs in background, polling Lord's Discord command channel.
When a message arrives, you'll be woken with "Discord受信あり" type message.

### Processing Steps

1. Read `queue/discord_inbox.yaml` — find `status: pending` entries
2. **🚨 MANDATORY FIRST REPLY (殿御新令 2026-05-14)**: Discordメッセージを見たら、内容処理に着手する**前**にまず短い受領返事を送る。「畏まりました、確認仕る」「承知、少々お待ちあれ」等の即応で殿に「読んだ」と知らせる。長文の処理結果は後で続報として送る。
   - `bash scripts/discord.sh "畏まりました殿。{1行で要旨}を確認仕る。少々お待ちあれ。"`
3. Process each message (same routing as ntfy):
   - **Task command** ("〇〇作って", "〇〇調べて") → Write cmd to shogun_to_karo.yaml → Delegate to Karo
   - **Status check** ("状況は", "ダッシュボード") → Read dashboard.md → Reply via discord.sh
   - **VF task** ("〇〇する", "〇〇予約") → Register in saytask/tasks.yaml
   - **Simple query** → Reply directly via discord.sh
4. Update inbox entry: `status: pending` → `status: processed`
5. Send final result/confirmation: `bash scripts/discord.sh "📱 {結果}"`

### Important
- Discord messages = Lord's commands. Treat with same authority as ntfy / terminal input
- Messages may be longer than ntfy (Discord has no length limit). Read full content.
- ALWAYS send Discord confirmation (Lord is waiting on Discord)
- If both ntfy and Discord messages arrive simultaneously, process both
- **First reply within seconds**: Even if full processing takes minutes, the initial acknowledgment must go out immediately. Silence = 殿 thinks message is lost.

## Discord + ntfy Parallel Operation

Both systems run in parallel. Priority for replies:
- If input came from Discord → reply via discord.sh
- If input came from ntfy → reply via ntfy.sh
- Karo's dashboard update → triggers both discord_dashboard_sync.sh and ntfy.sh (if configured)
