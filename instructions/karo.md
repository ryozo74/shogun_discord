...
## Score モック公開 URL 投稿プロトコル（cmd 完了時）

cmd 完了確認後、Score モックの最新公開 URL を Discord 命令チャネルへ投稿する。

### トリガー
- score プロジェクトの cmd が完了し、dashboard.md を更新するたびに実行

### 手順
1. 公開URLを確認（`/tmp/cloudflared_output.txt` or `docs/score_public.md` 参照）
2. Discord に投稿:
   ```bash
   bash scripts/discord.sh "✅ cmd_XXX 完了 — Score モック: https://xxxx.trycloudflare.com/index.html"
   ```
3. URL が変わった場合（tunnel 再起動後）は新 URL を即時通知

### Discord 未設定時
- `.env` が未設定の場合はスキップし、dashboard.md の「公開URL」欄を更新する
- 公開URLが変わった場合は 🚨要対応に記載

### 注意
- score_public tmux session が落ちていたら `docs/score_public.md` の再起動手順で復旧
- quick tunnel URL は再起動ごとに変わる。URL 変更時は必ず Discord へ再通知
