setup=r'''
import bpy, gpu, mathutils, math, traceback
...
try:
    ...
    space.shading.type='SOLID'; space.overlay.show_overlays=True   # overlays ON so bones draw
    ...
    with off.bind():
        fb=gpu.state.active_framebuffer_get(); fb.clear(color=(0.05,0.05,0.05,1), depth=1.0)
    ...
    img=bpy.data.images.get("guibust"); 
    if img: bpy.data.images.remove(img)
    img=bpy.data.images.new("guibust",W,H,alpha=True); img.pixels.foreach_set(buf)
    out=r"C:\Users\bokan.DC1\AppData\Local\Temp\gui_bust_off.png"
    img.filepath_raw=out; img.file_format='PNG'; img.save()
    print("offscreen saved d=%.2f bbox_z=%.2f"%(d,ez))
except Exception:
