...
try:
    ...
    # FACE close-up: frame on Head + Neck (head region)
    tp=[]
    for nm in ['Head','Neck']:
    ...
    tv=math.tan(math.atan(12.0/50.0)); th=tv*(1280/720); FILL=0.55   # head fills ~55%, room for face joints
    d=max((ez/2)/tv,(ey/2)/th)/FILL
    cam.location=bc+mathutils.Vector((-d,0,0))      # flipped side
    ...
    # project Head & Neck (the rest is outside)
    out={}
    for k,b in {'Nose':'Head','Neck':'Neck','RShoulder':'RightArm','LShoulder':'LeftArm'}.items():
        pb=arm.pose.bones.get(b)
        if pb:
            uv=world_to_camera_view(sc,cam_e,arm.matrix_world@pb.head); out[k]=[round(uv.x,4),round(uv.y,4)]
    json.dump({'kp':out},open(r"C:\Users\bokan.DC1\AppData\Local\Temp\facekp.json","w"))
    cuv=world_to_camera_view(sc,cam_e,bc)
    print("FACE close-up center_uv=(%.3f,%.3f) d=%.2f"%(cuv.x,cuv.y,d))
except Exception:
