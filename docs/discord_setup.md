...
ある将軍の発言に対し、別の将軍へ「これどう思う？」と論評させる運用に対応しています。

- 殿が **将軍 A のメッセージに Discord の返信（リプライ）をしつつ `@将軍B`** をメンションすると、
  将軍 B の `queue/discord_inbox.yaml` エントリに参照元が `reply_to` として付与されます:
...
- これにより将軍 B は「これ」が指す対象（将軍 A の所見）を文脈として受け取れます。
- リプライでない通常メッセージでは `reply_to` は付きません（後方互換）。
- 参照元は Discord がリプライ時に応答へ含める `referenced_message` から取得します。
  取得できない場合は `reply_to` を省略して通常処理を継続します（フェイルセーフ）。
