...
multi-agent-shogun は Discord 双方向通信に対応しています（cmd_417/429/438 で実装）。

#### クイックセットアップ
1. `.env.example` を `.env` にコピーし、Discord 認証情報を記入:
   ```
   ...
   ```
2. リスナー起動: `bash scripts/discord_listener.sh &`
3. メッセージ送信: `bash scripts/discord.sh "メッセージ"`
4. 会話履歴確認: `bash scripts/discord_conversation.sh`
