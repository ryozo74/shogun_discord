...
def render_and_post(token: str, channels: dict, state: dict,
    ...
    for name, root in fb.FLEETS.items():
        ...
        try:
            ...
            if not ch:
                # 設定で意図的に空 (分散型で他 daemon 担当等) なら silent skip
                # 真の「未設定」は起動時の load_channel_map で warning 済
                continue
            key = f"{name}_msg_id"
            new_id = post_or_edit(token, str(ch), gif_path, content, state, key)
            state[key] = new_id
            save_state(state)
            # 殿用: シェル上でも投稿内容を可視化 (tail -f で見られる)
            log(f"  [{name}] ch={ch} msg={new_id} | 作業:{cnt['work']} 待機:{cnt['idle']} "
                f"凍結:{cnt['freeze']} 未起動:{cnt['offline']} | GIF={gif_path}")
